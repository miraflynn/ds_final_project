...
df_states_racial_comb %>%
  ...
  group_by(date,state) %>%
  summarize(
    tot_cases = sum(cases, na.rm = TRUE)
  ) %>%
  ggplot() +
...
df_states_racial_comb %>%
  ...
  arrange(date) %>%
  mutate(
    delta_cases_per_100k = cases_per_100k - lag(cases_per_100k)
  ) %>%
  filter(race != "Other") %>%
  # filter(state == "Rhode Island") %>%
  filter(delta_cases_per_100k > 0) %>%
  filter(state == "Illinois") %>%
  ggplot() +
  geom_line(aes(x=date, y=delta_cases_per_100k, color=race)) +
  facet_wrap(~ state, scales = "free_y")
```
...
df_states_racial_comb %>%
  ...
  filter(race_estimate > 10000) %>%
  ggplot() +
  geom_boxplot(aes(x=race, y=cases_per_100k, color=race)) +
  facet_wrap(~ state, scales = "free_y")
```
...
df_states_racial_comb %>%
  ...
  mutate(
    cases_per_100k = cases/race_estimate
  ) %>%
   # filter(deaths_per_cases < .25) %>% 
ggplot(aes(x = date, y = cases_per_100k, color = race)) +
...
df_states_racial_comb %>%
  ...
  mutate(
    sum_cases = sum(cases),
    fraction_cases = cases/sum_cases
  ) %>%
  filter(!is.na(sum_cases)) %>%
  ggplot() +
  geom_line(aes(x=date,y=fraction_cases,color=race)) +
  facet_wrap(~ state) +
  theme_common() +
  theme(
    aspect.ratio = 9 / 16
...
#   group_by(date, state) %>%
#   mutate(
#     mean_dcases_per_100k = mean(delta_cases_per_100k, na.rm = TRUE),
#     diff_per100k_frommean = cases_per_100k - mean_dcases_per_100k
#   ) %>%
...
df_states_racial_comb %>%
  ...
  mutate(
    test_frac = cases/tot_cases,
    pop_frac = race_estimate/tot_tested_pop,
    cases_to_pop_ratio = test_frac / pop_frac
  ) %>%
  filter(cases_to_pop_ratio != 0) %>%
  ungroup() %>%
  ggplot() +
  geom_line(aes(x = date, y = cases_to_pop_ratio, color = race)) +
  facet_wrap(~ state)
```
...
df_states_racial_comb %>%
  ...
  arrange(date) %>%
  mutate(
    delta_cases_instant = cases - lag(cases),
    delta_cases_avg = mean_run(x = cases, k = 8, idx = date), #THIS IS WRONG!!!
    delta_cases = delta_cases_avg
  ) %>%
  ...
  group_by(date, state) %>%
  mutate(
    tot_dcases = sum(delta_cases, na.rm = TRUE),
    tot_tested_pop = sum(race_estimate, na.rm = TRUE)
  ) %>%
  group_by(date, state, race) %>%
  mutate(
    dtest_frac = delta_cases/tot_dcases,
    pop_frac = race_estimate/tot_tested_pop,
    cases_to_pop_ratio = dtest_frac / pop_frac
  ) %>%
  filter(cases_to_pop_ratio != 0) %>%
  ungroup() %>%
  ggplot() +
...
df_states_racial_comb %>%
  ...
  # pivot_longer(
  #   cols = c(cases_to_pop_ratio_avg, cases_to_pop_ratio_inst),
  #   names_to = "avg_or_inst",
  #   values_to = "cases_to_pop_ratio"
  # ) %>%
  # mutate(
  #   avg_or_inst = ifelse(avg_or_inst == "cases_to_pop_ratio_avg", "15 Day Average", "Instantaneous")
  # ) %>%
  ggplot() +
  geom_line(aes(x = date, y = cases_to_pop_ratio_inst, color = race), alpha = 0.2, size = 0.5) +
  # geom_point(aes(x = date, y = delta_cases_inst, color = race)) +
  geom_smooth(
    aes(x = date, y = cases_to_pop_ratio_inst, color = race),
    # method = "loess",
    span = 0.7,
    level = 0.8
  )+

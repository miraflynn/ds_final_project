...
df_states_racial_comb %>%
  ...
  mutate(
    ...
    delta_tests_avg = mean_run(x = tests, k = 8, idx = date),
...
df_states_racial_comb %>%
  ...
  # filter((abs(delta_tests_inst) / delta_tests_avg) < 2) %>%
  group_by(date, state, race) %>%
  mutate(
  ...
  mutate(
    avg_or_inst = ifelse(avg_or_inst == "tests_to_pop_ratio_avg", "15 Day Rolling Average", "Instantaneous")
  ) %>%
  ggplot() +
  geom_line(aes(x = date, y = tests_to_pop_ratio, color = race, alpha = avg_or_inst, size = avg_or_inst)) +
  scale_alpha_manual(values = c("Instantaneous" = 0.2, "15 Day Rolling Average" = 1)) +
  scale_size_manual(values = c("Instantaneous" = 0.5, "15 Day Rolling Average" = 1)) +
  facet_wrap(~ state) +
  ...
  labs(
    ...
    alpha = "Average or Instantaneous",
    size = "Average or Instantaneous",
    color = "Race"
  )
